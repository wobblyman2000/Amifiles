    ...
    switch (index) {
        ...
        case 1: // Layout & Tabs
            html += "<h2>2. Layout, Splits, &amp; Tabs</h2>"
                    ...
                    "<li><b>Group Multi-Disc Albums:</b> Right-click and check <i>Group Multi-Disc Albums</i> to automatically merge sibling disc folders (e.g. <code>cd1</code>, <code>CD 2</code>, <code>disc 3</code>, <code>d1</code>) under a parent directory into a single representing album card. The card displays the clean album name, and selecting it aggregates the tracks from all sibling discs in order inside the slide-out tracks list drawer. Double-clicking the card recursively plays the tracks of all discs.</li>"
                    "<li><b>Show Album Tracks Drawer:</b> Toggle this right-click context menu option to show or hide the sliding album tracks list drawer on the right. When hidden, selecting album cards will not open the drawer.</li>"
                    "<li><b>Double-click Action Options & Cover Art Engine:</b> Right-click and check whether double-clicking a media card/file starts playing it in the built-in fullscreen player or adds it to the playlist queue instead of interrupting playback. If neither option is checked, double-clicking will not auto-play or add to the queue. Theater View includes an expanded artwork scanner that searches all common image filenames (such as <code>front.jpg</code>, <code>album.jpg</code>, <code>art.jpg</code>, <code>jacket.jpg</code>, <code>disc.jpg</code>, <code>cd.jpg</code>, <code>thumb.jpg</code>) and performs wildcard image fallbacks across album and sibling disc folders to eliminate grey placeholder cards.</li>"
                    "<li><b>Auto Full Screen (20s Icon Button):</b> Toggle the vector-drawn <b>Auto FS</b> icon button on the media control bar (styled in Accent Blue when ON, Dim Gray when OFF) to automatically transition to borderless fullscreen playback after 20 seconds without manual intervention.</li>"
                    "<li><b>Hide Auxiliary / Artwork Files:</b> Right-click and check <i>Hide Auxiliary / Artwork Files</i> to automatically filter out non-essential companion files (such as <code>folder.jpg</code>, <code>cover.png</code>, subtitle files, XML metadata, and movie synopsis NFO files) under the hood inside Theater View. You can configure custom hide wildcard patterns (comma-separated list) in the <b>Preferences & System Settings -> Media Preview</b> page. Files are only hidden in Theater View to maintain a clutter-free showcase grid, remaining fully visible in other layout views.</li>"
                    "</ul>"
                    "</ul>"

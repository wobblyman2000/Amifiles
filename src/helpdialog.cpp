    ...
    switch (index) {
        ...
        case 1: // Layout & Tabs
            html += "<h2>2. Layout, Splits, &amp; Tabs</h2>"
                    ...
                    "<ul>"
                    "<li><b>Clean Interface (Zen Mode):</b> Right-click and check <i>Clean Interface Mode (Zen Mode)</i> or press <code>F11</code> (or select from the View menu) to hide all toolbars (including file operations and dynamic custom commands), menus, consoles, search bars, and status bars, leaving only the media cards and the preview panel. Right-click or press <code>F11</code> again to toggle back. Alt+Left/Right and Backspace keys allow for seamless, distraction-free keyboard navigation.</li>"
                    "<li><b>Built-in Fullscreen Playback:</b> Right-click and check <i>Double-click Plays Media in Built-in Fullscreen Player</i> to play media in the built-in media player and go fullscreen automatically on double-click. If a folder is double-clicked or played via the right-click menu, all media files inside (including nested multi-disc CD1, CD2, CD3, or compilation folders) are automatically scanned recursively and played in correct alphabetical/track order.</li>"
                    "<li><b>Dedicated Context Menu:</b> Hides standard file editing/clipboard actions (like Copy, Paste, Delete) to focus exclusively on media playback, tags, metadata scraping, Zen Mode toggles, and item properties.</li>"
                    "</ul>"
                    "</ul>"

    ...
    switch (index) {
        case 0: // Welcome & Overview
            html += "<h2>1. Welcome &amp; Overview</h2>"
                    ...
                    "<li><b>Commodore Disk Images (.d64, .d71, .d81, .g64):</b> <code>c1541</code> (part of the <b>VICE</b> emulator suite).</li>"
                    "<li><b>Amiga Disk Files (.adf, .adz):</b> <code>xdftool</code> (part of the Python-based <b>amitools</b> library; .adz requires <code>gzip</code>).</li>"
        ...
        case 1: // Layout & Tabs
            html += "<h2>2. Layout, Splits, &amp; Tabs</h2>"
                    ...
                    "</ul>"
                    "</ol>"
                    "<h3>Favorites, Bookmarks, &amp; Recents</h3>"
                    "<p><b>How to use:</b></p>"
                    "<ul>"
                    ...
                    "<li><b>Remove Favorite:</b> Right-click any entry inside the <b>Bookmarks</b> sidebar list to remove it.</li>"
                    "<li><b>Dynamic Recents Sidebar:</b> The <b>Recents</b> tab in the sidebar automatically tracks folders you visit in either panel. It keeps your last 15 visited local directories at your fingertips, moving duplicate paths to the top automatically and preserving history across application restarts. Double-click any recent directory to jump back to it immediately.</li>"
                    "</ul>"
                    ...
                    "</ul>"
                    "</ul>"
                    "<h3>Visual Executable Status Badge</h3>"
                    "<p>To distinguish executable scripts, installation packages, and binary utilities at a glance, Amifiles automatically scans file permission flags and prepends a terminal prompt icon badge (<code>[&gt;_]</code>) to the display name of any executable file. This indicator is visible across all layout view modes (Details, Cards, Miller columns, etc.).</p>"
                    ...
                    "<li><b>Music Showcase (Audio Theater):</b> Specialized for music collections and audio albums. Renders square CD Jewel Cases &amp; Vinyl LP sleeves, multi-disc album merging (CD1, CD2, Disc 3), Bottom Media Information Panel, batch ID3/FLAC tag editor, and audio visualizer support.</li>"
                    "<li><b>Cinema Showcase (Video Theater):</b> Dedicated to movies, TV series, documentaries, and video hubs. Renders 2:3 widescreen Movie Poster cards with DVD/Blu-Ray case borders, automatic video resolution badges (<i>4K UHD</i>, <i>1080p</i>, <i>720p</i>, <i>CINEMA</i>), auxiliary file hiding (*.nfo, *.srt, cover images), subtitle track selection, and Bottom Media Information Panel. Supports the following features:</li>"
                    "<ul>"
                    "<li><b>Clean Interface (Zen Mode):</b> Right-click and check <i>Clean Interface Mode (Zen Mode)</i> or press <code>F11</code> to hide all toolbars, menus, consoles, search bars, and status bars, leaving only the media player and Playlist Queue visible.</li>"
                    "<li><b>Built-in Fullscreen Playback:</b> Right-click and check <i>Double-click Plays Media in Built-in Fullscreen Player</i> to play media built-in and go fullscreen.</li>"
                    "<li><b>Resolution Badges:</b> Video cards automatically render high-contrast resolution badges (<i>4K UHD</i>, <i>1080p</i>, <i>720p</i>, <i>CINEMA</i>) floating in the top-right corner.</li>"
                    "<li><b>Hide Auxiliary / Artwork Files:</b> Toggle whether to filter out companion files (such as <code>folder.jpg</code>, subtitle files, XML metadata, and movie synopsis NFO files) under the hood inside Showcase Views. You can customize the specific file extensions to hide for either view separately via the right-click context menu.</li>"
                    "</ul>"
                    ...
                    "<li><b>Showcase View Grouping &amp; Collapsible Headers:</b> Group by Artist, Album, Genre, Type, Rating, or Custom Metadata across standard and Showcase views (v1 &amp; v2). Renders categorized sub-grids with collapsible group headers, item count badges, and one-click 'Play All' category buttons.</li>"
                    ...
                    "</ul>"
                     ...
                     "<p><b>How to use:</b></p>"
                     "<ul>"
                     "<li>Click on any empty space in Details, Grid, Cards, or Showcase views and drag the mouse cursor. A visible rubberband rectangle selection box will draw across the screen, selecting all files and folders that touch or fall inside the box.</li>"
                     "</ul>"
                     "<h3>Smart System Clipboard Paste (Images &amp; Text)</h3>"
                     "<p><b>How to use:</b></p>"
                     "<ul>"
                     "<li><b>Paste Clipboard Image:</b> Copy any raw image data to your system clipboard (from a web browser, screenshot utility, GIMP, or image editor) and press <code>Ctrl+V</code> inside Amifiles. Amifiles automatically creates and saves a new <code>clipboard.png</code> (or <code>clipboard_1.png</code>) image file in your active folder.</li>"
                     "<li><b>Paste Clipboard Text:</b> Copy raw text to your system clipboard from a web page, text editor, or terminal, and press <code>Ctrl+V</code> inside Amifiles. Amifiles automatically creates and saves a new <code>clipboard.txt</code> (or <code>clipboard_1.txt</code>) text document containing the copied string.</li>"
                     "<li><b>Standard File Copy/Paste:</b> Normal file and folder clipboard URL operations continue to operate as usual.</li>"
                     "</ul>"
                    ...
                    "<ul>"
                    "<li><b>Audio Showcase:</b> Designed for music collections with square CD jewel case styling, multi-disc CD grouping (<i>CD1</i>, <i>CD2</i>), Bottom Media Information Panel, and ID3 metadata integration. Automatically hides loose artwork images and video files for a clean music collection view.</li>"
                    "<li><b>Video Showcase:</b> Tailored for movies and TV shows with 2:3 vertical video poster cards, multi-level series drill-down (<i>Show</i> -&gt; <i>Season</i> -&gt; <i>Episodes</i>), dedicated <b>SEASON 01</b> badges, season artwork scanner, video resolution badges (4K UHD, 1080p, 720p), and Bottom Media Information Panel. Automatically hides loose artwork images, audio files, companion <code>.xml</code>, <code>.nfo</code>, and <code>theme.mp3</code> files for a pristine TV and Movie library view.</li>"
                    "<li><b>Movie Showcase (v2):</b> A premium Netflix-style layout with 16:9 widescreen movie backdrops, glassmorphic bottom info overlays with a prominent orange <b>Play</b> button and a <b>More Info</b> button, star ratings (e.g. ⭐⭐⭐⭐☆), genre/release tags, and full-screen ambient backdrop gradients.</li>"
                    "<li><b>TV Show Showcase (v2):</b> An AppleTV+/Disney+-style layout with 16:9 widescreen banners, season-based folder grouping, progress bars showing episode watch stats, green <b>WATCHED ✔</b> badges, interactive episode detail slideouts, and automatic theme music auto-playback when navigating folders.</li>"
                    "<li><b>Music Showcase (v2):</b> A modern Spotify-style layout with square album art, 3D vinyl record disc previews, hover play buttons, quick-track play lists, interactive queueing controls, and color-matched ambient glow.</li>"
                    "</ul>"
                    "<li><b>Subfolder Depth Inheritance:</b> Profiles feature a configurable subfolder depth level (0 = exact folder only, 1-3 = nested levels, Unlimited = all subfolders). For TV Show libraries, set depth to <b>3 Subfolder Levels Deep</b> or <b>Unlimited</b> so navigating into <code>TV Shows</code> -&gt; <code>Show Name</code> -&gt; <code>Season 01</code> maintains the Video Showcase layout without reverting views.</li>"
                    "<li><b>Reset All Settings to Defaults:</b> Located in <b>Settings -&gt; Preferences</b>. Click the <b>🔄 Reset All to Defaults</b> button at any time to instantly restore factory default settings, toolbar configurations, and rules without deleting configuration files.</li>"
                    "<li><b>Artwork Standardization Helper:</b> Includes an automated helper script (<code>scripts/fix_media_artwork.py</code>) to scan and standardize TV series and movie artwork filenames (<code>poster.jpg</code>, <i>fanart.jpg</i>, <i>banner.jpg</i>, <i>season01.jpg</i>).</li>"
                    "<li><b>Drag &amp; Drop Policy:</b> Drag and drop is fully enabled across all standard view modes (Details Table, Grid/Icons, Card/Tiles, Miller Columns, Chronological Timeline, and Filmstrip). Drag and drop is strictly disabled inside <b>Audio Showcase</b> and <b>Video Showcase</b> views to prevent accidental file moves or crashes while browsing media libraries.</li>"
                    ...
                    "<li><b>TV Series Theme Music Auto-Playback:</b> In <b>Video Showcase</b> mode, navigating into any TV series folder automatically searches for <code>theme.mp3</code> (or <code>theme.ogg</code>/<code>theme.flac</code>) and plays background TV theme music. Theme songs play through exactly once from start to finish and stop automatically without endless looping. Moving to another TV show folder seamlessly plays the new show's theme song. Configurable in <b>Settings -&gt; Preferences</b>.</li>"
                    "<li><b>Zen Mode Theme-Linked Background Music Loop:</b> When <b>Clean Interface (Zen Mode)</b> is active, Amifiles checks the current theme configuration (via Theme preset or custom setting) and looks for a corresponding background track MP3 (e.g. <code>catppuccin_mocha.mp3</code>, <code>theme.mp3</code>, <code>zen.mp3</code>) inside <code>~/.config/Amifiles/themes/</code> or <code>~/.local/share/Amifiles/themes/</code> to play in a loop at 50% volume. Playing any track in the main media player automatically pauses the theme background music, and resuming or stopping the main player automatically unpauses the background theme loop.</li>"
                    "<li><b>Breadcrumb Address Bar &amp; Sibling Folder Dropdown:</b> The address bar renders interactive Dolphin-style path breadcrumb segments. Click any segment to jump directly to that parent folder level, click the dropdown arrow <code>▾</code> beside any path segment to open a find-as-you-type searchable filter menu of sibling folders, or click empty space to type manual path strings.</li>"
                    "<li><b>Media Info Sheet Overlay &amp; Synopsis Parser:</b> Right-click any poster card and select <b>🎬 Media Info Sheet... (ℹ)</b> to open a glassmorphism dialog displaying high-resolution artwork, technical resolution/format specs, and plot synopsis automatically parsed from local <code>.nfo</code>, <code>movie.nfo</code>, <code>tvshow.nfo</code>, or <code>summary.txt</code> files, loaded inside a sleek, scrollbar-customized read-only text browser.</li>"
                    "<li><b>TV Episode Watch Progress &amp; Badges:</b> Toggle watch status directly from the right-click menu or Info Sheet dialog (**Mark as Watched** / **Unwatched**). Displays green **<code>WATCHED ✔</code>** badges on poster cards in Video Showcase mode, persisted across application restarts.</li>"
                    "<li><b>Dynamic Ambient Blurred Artwork Backgrounds:</b> When browsing Showcase libraries, Amifiles extracts the dominant color from folder artwork (<code>poster.jpg</code>, <code>cover.jpg</code>, <code>folder.jpg</code>) to generate a smooth, ambient background gradient glow behind media cards.</li>"
        ...
        case 4: // Media Preview & Covers
            html += "<h2>5. Media Preview &amp; Docks</h2>"
                    ...
                    "<li><b>Interactive Image Editor &amp; Annotator:</b> Highlight any image and click <b>Edit Image &amp; Annotate</b> inside the preview panel. This launches a graphics workspace with powerful annotation tools (drawing shapes, highlights, text, cropping, rotation).</li>"
                    "<ul>"
                    "<li><b>Real-Time Chroma Key (Green Screen Removal):</b> Click the green screen icon (🟢) to transparentize green screens.</li>"
                    "<li><b>Live Sensitivity Tuning:</b> Drag the threshold slider next to the button to fine-tune the green transparency sensitivity from 10% to 90% dynamically with live preview feedback before saving.</li>"
                    "</ul>"
                    "</ul>"
        ...
        case 6: // Disk Utilities
            html += "<h2>7. Disk Utilities &amp; Premium Tools</h2>"
                    ...
                    "</ol>"
                    "<h3>File Permissions &amp; Executables (chmod)</h3>"
                    "<p>Amifiles makes it simple to inspect, adjust, and toggle executable permissions for scripts and application binaries directly from the application:</p>"
                    "<ul>"
                    "<li><b>Inspect/Change Permissions:</b> Right-click any file or folder and select <b>Change File Permissions (chmod)...</b>. This launches a permissions editor window allowing you to view and toggle Read, Write, and Execute bits independently for Owner, Group, and Others.</li>"
                    "<li><b>Toggle Executable:</b> Right-click any file and select <b>Toggle Executable Status</b> to instantly add or remove execute permissions for the current user. Files with executable bits enabled will automatically show a <code>[&gt;_]</code> terminal status badge.</li>"
                    "</ul>"
                    "<h3>Password-Protected Archives &amp; ADF/D64 VFS</h3>"
                    "<p><b>How to use:</b></p>"
                    "<ul>"
                    "<li><b>Mount retro disks:</b> Double-click any Amiga <code>.adf</code>/<code>.adz</code>, Commodore <code>.d64</code>/<code>.d71</code>/<code>.d81</code>/<code>.g64</code>, CD/DVD <code>.iso</code>, or partition <code>.img</code>. The panel opens a virtual filesystem.</li>"
                    ...
                    "</ol>"
                    "<h3>Folder Profiles &amp; Layouts Manager</h3>"
                    "<p><b>How to use:</b></p>"
                    "<ol>"
                    "<li>Select **Settings -> Folder Profiles &amp; Layouts -> Folder Profiles &amp; Layouts...** (or under Favorites).</li>"
                    "<li>To create a new profile: click **Add**, enter a descriptive name, and configure the target folder path or category mapping.</li>"
                    "<li>**Cloning Current UI**: Click **Capture Current UI State** to copy active window geometries, view modes, and visible dock panels instantly.</li>"
                    "<li>**Slide Toggle Switches**: Enforce visibility overrides, auto-apply activation, and visualizer settings using intuitive modern slide toggle switch widgets.</li>"
                    "<li>**Tab Snapshots**: Toggle **Include Open Tabs Snapshot** and click **Capture Current Open Tabs** to save your active tab lists. Use **Apply Profile to Current View Now** to load them manually on demand.</li>"
                    "<li>**Saving &amp; Auto-Activation**: Click **Save &amp; Apply Profiles**. If **Auto-apply** is switched on, the layout, styling, and background color will dynamically activate whenever you browse inside the target path.</li>"
                    "<li>**Bypassing Layout Rules**: If you need to debug layout conflicts, select **Settings -> Bypass Folder Profiles** to temporarily ignore all active rules globally.</li>"
                    "<li>**Smart Directory Playback**: Double-clicking a directory in Theater View (or when folder-level double-click playback is active) will automatically scan and play its contents recursively in Fullscreen *if* the directory is a playable album (directly containing audio/video files or multi-disc subdirectories like `CD1`, `CD2`, `Disc 1`). Non-media folders (such as artist indexes or parent TV series directories) will drill down normally. You can also right-click any folder and select **Play Folder/Album in Fullscreen** to trigger playback on demand.</li>"
                    "</ol>"
        ...
        case 8: // Settings & Preferences
            html += "<h2>9. Settings &amp; Preferences</h2>"
                    ...
                    "<h3>Customizable Context Menus</h3>"
                    "<p>Amifiles supports full customization of the right-click file context menus. To customize:</p>"
                    "<ol>"
                    "<li>Navigate to <b>Settings -> Toolbars &amp; Hotkeys -> Configure Context Menu...</b>.</li>"
                    "<li>Add, remove, rearrange, or edit submenus, separators, and actions using the drag-and-drop tree editor.</li>"
                    "<li><b>Reset to Defaults:</b> If you want to undo your customizations and revert to the built-in system menus, click the <b>Reset to Defaults 🔄</b> button at the bottom of the editor. This clears your custom configurations and reloads the default layout immediately.</li>"
                    "</ol>";
            break;
        ...
        case 9: // Automation & Rules
            html += "<h2>10. Automation &amp; Rules</h2>"
                    ...
                    "<h3>Dynamic Folder-Specific Layouts &amp; Custom Wallpapers</h3>"
                    "<p>Define rule chains that automatically switch view modes (e.g. Details, Chronological Timeline, Card Grid) and show/hide toolbars when you enter specific directories. This is useful for automatically showing grids and CD cases inside music folders, and details tables inside documents directories. You can also assign distinct background colors or wallpapers for any directory:</p>"
                    "<ul>"
                    "<li><b>Custom Folder Wallpapers:</b> Set layout rules to load a custom background image/wallpaper when you enter a specific directory path or category.</li>"
                    "<li><b>Wallpaper Opacity/Dimming:</b> If a wallpaper is too bright, you can adjust the <b>Wallpaper Opacity</b> slider in the layout editor. This dynamically overlays a semi-transparent theme backdrop (alpha mask) over the wallpaper inside the file panels, ensuring text filenames remain highly visible.</li>"
                    "</ul>"
